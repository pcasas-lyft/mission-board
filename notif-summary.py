...
def main():
    ...
    # Accept multiple branches: argv[2], argv[3], ... (old callers pass one branch + task_id_file)
    # New callers (on-stop.sh) pass all branches as separate args after the path.
    branches = [b.strip() for b in sys.argv[2:] if b.strip() and not b.startswith('/')]
    ...
    # Aggregate task IDs from session files across all branches
    seen = set()
    task_ids = []
    ...
    # Fallback: claimedBy on primary branch
    if not session_tasks and branches:
        branch = branches[0]
